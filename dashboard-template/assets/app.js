...
function renderKPIs(data) {
  ...
  const items = [
    ...
    ['Safety', `${s.safety_margin_band} (${s.safety_margin_score})`],
    ['Commit Win', s.intake_commitment_window],
    ['Scope Lock', s.scope_lock_state],
  ];
...
function renderQuality(data) {
  ...
  document.getElementById('quality').innerHTML = `
    ...
    <p>Policy Compliance: ${badge(data.status.policy_compliance)} | Confidence: ${badge(data.status.execution_confidence_band)} | Mode: ${badge(data.status.execution_mode)} | Momentum: ${badge(data.status.momentum_posture)} (${data.status.momentum_index}) | Pressure: ${badge(data.status.pressure_level)} (${data.status.pressure_index}) | Temperature: ${badge(data.status.temperature)} (${data.status.temperature_index}) | Control: ${badge(data.status.control_mode)} | Efficiency: ${badge(data.status.efficiency_band)} (${data.status.efficiency_score}) | Intervention: ${badge(data.status.intervention_mode)} | Friction: ${badge(data.status.friction_band)} (${data.status.friction_score}) | Cadence: ${badge(data.status.cadence)} | Focus: <b>${data.status.focus_items}</b> | Owners: <b>${data.status.owners_in_scope}</b> | Overload: <b>${data.status.overloaded_owners}</b> | Throttle: ${badge(data.status.throttle_mode)} | Corridor: ${badge(data.status.priority_corridor)} | Queue: ${badge(data.status.queue_pressure_band)} (${data.status.queue_pressure_score}) | Bandwidth: ${badge(data.status.bandwidth_mode)} (${data.status.bandwidth_score}) | Intake: ${badge(data.status.intake_guard)} | Capacity: ${badge(data.status.intake_capacity_mode)} (${data.status.intake_capacity_score}) | Admission: ${badge(data.status.admission_state)} | Commitment: ${badge(data.status.commitment_mode)} | Scope: ${badge(data.status.scope_budget_mode)} (${data.status.scope_budget_score}) | Window: ${badge(data.status.admission_window)} | Guard: ${badge(data.status.commitment_guard)} | Risk Budget: ${badge(data.status.risk_budget_mode)} (${data.status.risk_budget_score}) | Sync: ${badge(data.status.delivery_intake_sync)} | Reserve: ${badge(data.status.execution_reserve)} | Buffer: ${badge(data.status.capacity_buffer_band)} (${data.status.capacity_buffer_score}) | Queue Policy: ${badge(data.status.intake_queue_policy)} | Rebalance: ${badge(data.status.scope_rebalance)} | Flow: ${badge(data.status.flow_control_mode)} (${data.status.flow_control_score}) | Release Win: ${badge(data.status.intake_release_window)} | Stability: ${badge(data.status.execution_stability_guard)} | Safety: ${badge(data.status.safety_margin_band)} (${data.status.safety_margin_score}) | Commit Win: ${badge(data.status.intake_commitment_window)} | Scope Lock: ${badge(data.status.scope_lock_state)} | Validation rate: <b>${v.valid_rate}%</b></p>
